...
- **Single Sign-On:** Sign in with an account you already use. Parcourse supports OIDC directly, so the same four settings work with `Zitadel`, `Keycloak`, `Auth0`, `Entra`, `Okta`, and `Google`. Layer `docker-compose.oidc.yml` over whichever compose file you already run. The button only appears when it has been set up, and the password sign-in form stays exactly where it was.
