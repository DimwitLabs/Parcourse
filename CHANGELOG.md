...
- **One Way In:** Set `OIDC_ONLY=true` and your provider becomes the only way to sign in. Off by default, and Parcourse refuses to start if you turn it on without a provider configured.
  - The password form goes from the login screen, **Settings** stops offering to change a password, and an admin cannot set one, because a password on such an instance could never be used.
  - Suspending somebody at your provider stops them signing in to Parcourse as well, though a session they already hold lasts until it expires (24 hours by default).
  - The login screen hands you straight to the provider, and signing out keeps you signed out until you choose to sign in again.
  - An existing account is matched by email even when the provider does not mark the address as verified, since the provider is the only one vouching for anyone.
...
- **Brand Package:** The stylesheet, colours and logos now live in `@dimwitlabs/parcourse-brand`, published to GitHub Packages with every release, so anything built alongside Parcourse can look exactly like it.
...
- **Upgrades Show Up Straight Away:** The frontend now tells browsers to check for a new page on every visit, so an upgrade is no longer hidden behind a cached copy of the old one.
