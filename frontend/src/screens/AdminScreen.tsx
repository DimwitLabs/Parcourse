...
export default function AdminScreen() {
  ...
  // Whoever runs the instance is not one of the people using it, so the two
  // are listed apart rather than sorted together.
  const admins = users.filter((u) => u.role === "admin");
  const students = users.filter((u) => u.role !== "admin");

  function userRow(u: UserWithUsage) {
      ...
      <div className="user-row" key={u.id}>
        ...
        {isConfirming ? (
          <div className="confirm-bar">
            ...
            <button
              className={`icon-btn tip${confirm.action === "delete" ? " danger" : " primary"}`}
              data-tip="Confirm"
              aria-label="Confirm"
              onClick={() =>
            ...
            <button className="icon-btn tip" data-tip="Cancel" aria-label="Cancel" onClick={() => setConfirm(null)}>
  ...
  return (
    <div className="admin-view">
      ...
      <span className="user-group-label">Admin</span>
      <div className="card user-table">{admins.map(userRow)}</div>

      {students.length > 0 && (
        <>
          <span className="user-group-label">Users</span>
          <div className="card user-table">{students.map(userRow)}</div>
        </>
      )}
    </div>
  );
