...
export default function HomeScreen() {
  ...
  async function createCourse() {
    try {
      setStep("transcript");
      toast("Extracting transcript…", "info");
      ...
      setStep("generating");
      toast("Generating course — this may take a minute…", "info");
  ...
  return (
    <>
      <div className="hero">
        ...
        {busy && (
          <p className="status-message generation-msg">
            {step === "generating" && funMsg ? funMsg : step === "transcript" ? "Extracting transcript…" : step === "guardrail" ? "Checking content…" : "Working…"}
          </p>
        )}
      </div>
            ...
            {recentCourses.map((c) => (
              <Link to={`/course/${c.id}`} key={c.id} className="course-card">
                ...
                <h4 className="course-card-title">
                  {c.sections[0]?.title ?? "Untitled course"}
                </h4>
